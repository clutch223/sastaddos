...
# Database Path (Railway Persistence ke liye)
db_path = 'sastadev_users.json'
db = TinyDB(db_path)
User = Query()
...
async def attack_progress_task(context, chat_id, message_id, target, port, duration, test_id):
    ...
    while remaining > 0:
        ...
        wait_time = min(15, remaining) # Update every 15s for Railway stability
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("❌ `/genkey hours 2`")
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if found:
        ...
        await update.message.reply_text(f"✅ **Success!** Expiry: `{expiry}`")
